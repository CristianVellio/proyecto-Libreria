...
const ManipularLibros = () => {
  ...
  const { data: libros, refetch } = useFetchAllLibrosQuery();

  const [eliminarLibro] = useDeleteLibroMutation();

  const handleEliminarLibro = async (id) => {
    try {
      await eliminarLibro(id).unwrap();
      alert("Libro borrado con exito!");
      refetch();
    } catch (error) {
      console.error("No se pudo borrar el libro:", error.message);
      alert("No se pudo borrar el libro. Intenta otra vez.");
    }
  };

  const handleEditarLibro = (id) => {
    navigate(`tablero/editar-libro/${id}`);
  };
  return (
                  ...
                  libros.map((Libro, index) => (
                    <tr key={index}>
                      ...
                      <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 space-x-4">
                        <Link
                          to={`/tablero/editar-libro/${Libro._id}`}
...
export default ManipularLibros;
